...
do
    ...
    function BaseGroupboxFuncs:AddPlayerList(Idx, Info)
        Info = Info or {}
        ...
        local PlayerList = {
            ...
            Disabled = if typeof(Info.Disabled) == "boolean" then Info.Disabled else false,
            Players = {},
            StatusValues = if typeof(Info.StatusValues) == "table" then Info.StatusValues else { "None" },
            Selected = nil,
        ...
        local function ReadPlayers()
            if typeof(Info.GetPlayers) == "function" then
                local Success, Result = pcall(Info.GetPlayers)
                if Success and typeof(Result) == "table" then
                    return Result
                end
            end

            if typeof(Info.Players) == "table" then
                return Info.Players
            elseif typeof(PlayerList.Players) == "table" and #PlayerList.Players > 0 then
        ...
        local function FormatPlayerName(Player)
            if typeof(Info.FormatName) == "function" then
                local Success, Result = pcall(Info.FormatName, Player)
            ...
            elseif typeof(Player) == "table" and Player.Name ~= nil then
                return tostring(Player.Name)
            end

            return Player and tostring(Player) or "None"
        end

        local function GetTeamText(Player)
            if typeof(Info.GetTeam) == "function" then
                local Success, Result = pcall(Info.GetTeam, Player)
                if Success and Result ~= nil then
                    return tostring(Result)
                end
            end
            ...
            if typeof(Player) == "Instance" and Player:IsA("Player") then
                if LocalPlayer and Player.TeamColor == LocalPlayer.TeamColor then
                    return "Friendly"
                end
        ...
        local function GetAvatar(Player)
            if typeof(Info.GetAvatar) == "function" then
                local Success, Result = pcall(Info.GetAvatar, Player)
            ...
            if typeof(Player) == "Instance" and Player:IsA("Player") then
                if AvatarCache[Player.UserId] then
                    return AvatarCache[Player.UserId]
                end
                ...
                task.spawn(function()
                    ...
                    if Success and Thumbnail then
                        AvatarCache[Player.UserId] = Thumbnail
                        if PlayerList.Selected == Player and PlayerList.SetAvatar then
                            PlayerList:SetAvatar(Thumbnail)
                        end
                    end
                end)
            end
        ...
        local function NormalizeStatus(Status)
            ...
            for _, Value in ipairs(PlayerList.StatusValues) do
                if tostring(Value) == Status then
                    return Status
                end
            end

            return tostring(PlayerList.StatusValues[1] or "None")
        end

        local function FireStatusChanged()
            if PlayerList.Selected then
                Library:SafeCallback(PlayerList.OnStatusChanged, PlayerList.Selected, PlayerList.SelectedStatus)
            end
        end

        local Holder = Library:Create("Frame", {
            BackgroundTransparency = 1;
            Size = UDim2.new(1, -4, 0, HolderHeight);
        ...
        Library:AddToRegistry(PanelOuter, { BorderColor3 = "Black"; })
        Library:AddToRegistry(PanelInner, { BackgroundColor3 = "MainColor"; BorderColor3 = "OutlineColor"; })

        local Title = Library:CreateLabel({
            BackgroundTransparency = 1;
            Position = UDim2.new(0, 7, 0, 3);
            Size = UDim2.new(1, -14, 0, 17);
            Text = Info.Text or "Player List";
            TextSize = 14;
            TextXAlignment = Enum.TextXAlignment.Left;
            ZIndex = 7;
            Parent = PanelInner;
        ...
        local TableOuter = Library:Create("Frame", {
            ...
            Size = UDim2.new(1, -14, 0, TableHeight);
            ZIndex = 7;
            Parent = PanelInner;
        })

        local TableInner = Library:Create("Frame", {
            BackgroundColor3 = Library.BackgroundColor;
            BorderColor3 = Library.OutlineColor;
            BorderMode = Enum.BorderMode.Inset;
            Size = UDim2.fromScale(1, 1);
        ...
        Library:AddToRegistry(TableOuter, { BorderColor3 = "Black"; })
        Library:AddToRegistry(TableInner, { BackgroundColor3 = "BackgroundColor"; BorderColor3 = "OutlineColor"; })

        local Header = Library:Create("Frame", {
            BackgroundTransparency = 1;
            Size = UDim2.new(1, 0, 0, TableHeaderHeight);
            ZIndex = 9;
            Parent = TableInner;
        })

        local function CreateColumnLabel(Parent, Position, Size, Text, ZIndex)
            local Label = Library:CreateLabel({
                BackgroundTransparency = 1;
                Position = Position;
                Size = Size;
        ...
        CreateColumnLabel(Header, UDim2.new(0, 8, 0, 1), UDim2.new(0.34, -12, 1, 0), "Name")
        CreateColumnLabel(Header, UDim2.new(0.34, 8, 0, 1), UDim2.new(0.33, -12, 1, 0), "Team")
        CreateColumnLabel(Header, UDim2.new(0.67, 8, 0, 1), UDim2.new(0.33, -12, 1, 0), "Status")

        local RowHolder = Library:Create("Frame", {
            BackgroundTransparency = 1;
            Position = UDim2.new(0, 0, 0, TableHeaderHeight);
            Size = UDim2.new(1, 0, 1, -TableHeaderHeight);
            ZIndex = 9;
            Parent = TableInner;
        })
        ...
        for RowIndex = 1, RowCount do
            ...
            local TeamLine = Library:Create("Frame", {
                BackgroundColor3 = Library.OutlineColor;
                BorderSizePixel = 0;
                Position = UDim2.new(0.34, 0, 0, 4);
                Size = UDim2.new(0, 1, 1, -8);
                ZIndex = 11;
                Parent = RowButton;
            })

            local StatusLine = Library:Create("Frame", {
                BackgroundColor3 = Library.OutlineColor;
                BorderSizePixel = 0;
                Position = UDim2.new(0.67, 0, 0, 4);
                Size = UDim2.new(0, 1, 1, -8);
                ZIndex = 11;
                Parent = RowButton;
            })

            local NameLabel = CreateColumnLabel(RowButton, UDim2.new(0, 8, 0, 1), UDim2.new(0.34, -12, 1, -2), "", 12)
            local TeamLabel = CreateColumnLabel(RowButton, UDim2.new(0.34, 8, 0, 1), UDim2.new(0.33, -12, 1, -2), "", 12)
            local StatusLabel = CreateColumnLabel(RowButton, UDim2.new(0.67, 8, 0, 1), UDim2.new(0.33, -12, 1, -2), "", 12)

            Library:AddToRegistry(RowButton, { BackgroundColor3 = "BackgroundColor"; })
            Library:AddToRegistry(BottomLine, { BackgroundColor3 = "OutlineColor"; })
            Library:AddToRegistry(TeamLine, { BackgroundColor3 = "OutlineColor"; })
            Library:AddToRegistry(StatusLine, { BackgroundColor3 = "OutlineColor"; })
            Library:OnHighlight(RowButton, RowButton, { BackgroundColor3 = "MainColor" }, { BackgroundColor3 = "BackgroundColor" })
            ...
            RowButton.MouseButton1Click:Connect(function()
                if PlayerList.Disabled or not Row.Player then
                    return
                end

                PlayerList:SetSelected(Row.Player)
            end)

            RowObjects[RowIndex] = Row
        end

        local function CreateScrollButton(Text, Position, Delta)
            local Button = Library:Create("TextButton", {
                AutoButtonColor = false;
                BackgroundTransparency = 1;
                Position = Position;
            ...
            Button.MouseButton1Click:Connect(function()
                ...
                PlayerList:SetScroll(PlayerList.ScrollOffset + Delta)
            end)
        ...
        local AvatarOuter = Library:Create("Frame", {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Position = UDim2.new(0, 7, 0, BottomY - 10);
        ...
        Library:AddToRegistry(AvatarOuter, { BorderColor3 = "Black"; })
        Library:AddToRegistry(AvatarInner, { BackgroundColor3 = "BackgroundColor"; BorderColor3 = "OutlineColor"; })
        ...
        local StatusLabel = CreateColumnLabel(StatusInner, UDim2.new(0, 7, 0, 0), UDim2.new(1, -28, 1, 0), PlayerList.SelectedStatus, 11)
        local StatusArrow = CreateColumnLabel(StatusInner, UDim2.new(1, -22, 0, -1), UDim2.new(0, 16, 1, 0), "-", 11)
        StatusArrow.TextXAlignment = Enum.TextXAlignment.Center

        Library:AddToRegistry(StatusOuter, { BorderColor3 = "Black"; })
        Library:AddToRegistry(StatusInner, { BackgroundColor3 = "MainColor"; BorderColor3 = "OutlineColor"; })
        Library:OnHighlight(StatusOuter, StatusOuter, { BorderColor3 = "AccentColor" }, { BorderColor3 = "Black" })

        local StatusListOuter = Library:Create("Frame", {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Visible = false;
            ZIndex = 40;
        ...
        local StatusListInner = Library:Create("Frame", {
            BackgroundColor3 = Library.MainColor;
            BorderColor3 = Library.OutlineColor;
            BorderMode = Enum.BorderMode.Inset;
            Size = UDim2.fromScale(1, 1);
            ZIndex = 41;
            Parent = StatusListOuter;
        })

        Library:AddToRegistry(StatusListOuter, { BorderColor3 = "Black"; })
        Library:AddToRegistry(StatusListInner, { BackgroundColor3 = "MainColor"; BorderColor3 = "OutlineColor"; })

        local function RepositionStatusDropdown()
            StatusListOuter.Position = UDim2.fromOffset(StatusOuter.AbsolutePosition.X, StatusOuter.AbsolutePosition.Y + StatusOuter.AbsoluteSize.Y + 1)
            StatusListOuter.Size = UDim2.fromOffset(StatusOuter.AbsoluteSize.X, math.max(#PlayerList.StatusValues, 1) * 20)
        end

        local function CloseStatusDropdown()
            StatusListOuter.Visible = false
            Library.OpenedFrames[StatusListOuter] = nil
        end

        local function BuildStatusDropdown()
            for _, Child in ipairs(StatusListInner:GetChildren()) do
                Child:Destroy()
            end

            for Index, Status in ipairs(PlayerList.StatusValues) do
                local StatusButton = Library:Create("TextButton", {
                    AutoButtonColor = false;
                    BackgroundColor3 = Library.MainColor;
                    BorderColor3 = Library.OutlineColor;
                    BorderMode = Enum.BorderMode.Middle;
                    Position = UDim2.new(0, 0, 0, (Index - 1) * 20);
                    Size = UDim2.new(1, -1, 0, 20);
                    Text = "";
                    ZIndex = 42;
                    Parent = StatusListInner;
                })

                local StatusButtonLabel = CreateColumnLabel(StatusButton, UDim2.new(0, 6, 0, 0), UDim2.new(1, -12, 1, 0), tostring(Status), 43)

                Library:AddToRegistry(StatusButton, { BackgroundColor3 = "MainColor"; BorderColor3 = "OutlineColor"; })
                Library:OnHighlight(StatusButton, StatusButton, { BorderColor3 = "AccentColor" }, { BorderColor3 = "OutlineColor" })
                ...
                StatusButton.MouseButton1Click:Connect(function()
                    ...
                    PlayerList.SelectedStatus = NormalizeStatus(Status)
        ...
        Library:GiveSignal(StatusOuter.InputBegan:Connect(function(Input)
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                if StatusListOuter.Visible then
                    CloseStatusDropdown()
                else
                    RepositionStatusDropdown()
                    StatusListOuter.Visible = true
        ...
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                local AbsPos, AbsSize = StatusListOuter.AbsolutePosition, StatusListOuter.AbsoluteSize
                local InList = Mouse.X >= AbsPos.X and Mouse.X <= AbsPos.X + AbsSize.X and Mouse.Y >= AbsPos.Y and Mouse.Y <= AbsPos.Y + AbsSize.Y
                local StatusPos, StatusSize = StatusOuter.AbsolutePosition, StatusOuter.AbsoluteSize
                local InButton = Mouse.X >= StatusPos.X and Mouse.X <= StatusPos.X + StatusSize.X and Mouse.Y >= StatusPos.Y and Mouse.Y <= StatusPos.Y + StatusSize.Y
        ...
        local function CreateSmallButton(Text, Position, Size, Callback)
            ...
            ButtonOuter.MouseButton1Click:Connect(function()
                ...
                Library:SafeCallback(Callback, PlayerList.Selected)
            end)
        ...
        function PlayerList:SetStatusValues(NewValues)
            if typeof(NewValues) == "table" and #NewValues > 0 then
                PlayerList.StatusValues = NewValues
                PlayerList.SelectedStatus = NormalizeStatus(PlayerList.SelectedStatus)
                StatusLabel.Text = PlayerList.SelectedStatus
                BuildStatusDropdown()
            end
        end

        function PlayerList:SetAvatar(Image)
            AvatarImage.Image = typeof(Image) == "string" and Image or ""
        end

        function PlayerList:SetSelectedText(Text)
        ...
        function PlayerList:SetSelected(Player, Silent)
            ...
            for _, Row in ipairs(RowObjects) do
                SetRowSelected(Row, Row.Player == PlayerList.Selected)
            end

            if not Silent then
                Library:SafeCallback(PlayerList.OnSelect, Player)
            end
        end

        function PlayerList:SetScroll(Offset)
            PlayerList.ScrollOffset = math.max(math.floor(tonumber(Offset) or 0), 0)
            PlayerList:Refresh()
        end

        function PlayerList:Refresh()
            PlayerList.Players = ReadPlayers()
            local VisiblePlayers = {}

            for _, Player in ipairs(PlayerList.Players) do
                if not (Info.ExcludeLocalPlayer and Player == LocalPlayer) then
                    table.insert(VisiblePlayers, Player)
                end
            end

            if PlayerList.Selected and not table.find(VisiblePlayers, PlayerList.Selected) then
            ...
            local MaxOffset = math.max(#VisiblePlayers - RowCount, 0)
            PlayerList.ScrollOffset = math.clamp(PlayerList.ScrollOffset, 0, MaxOffset)

            for RowIndex = 1, RowCount do
                local Row = RowObjects[RowIndex]
                local Player = VisiblePlayers[RowIndex + PlayerList.ScrollOffset]
            ...
            local CanScroll = #VisiblePlayers > RowCount
            ScrollUpButton.Visible = CanScroll and PlayerList.ScrollOffset > 0
            ScrollUpLabel.Visible = ScrollUpButton.Visible
            ScrollDownButton.Visible = CanScroll and PlayerList.ScrollOffset < MaxOffset
            ScrollDownLabel.Visible = ScrollDownButton.Visible
        end

        function PlayerList:SetVisible(Visibility)
            PlayerList.Visible = Visibility
            Holder.Visible = PlayerList.Visible
            if Blank then Blank.Visible = PlayerList.Visible end
        ...
        function PlayerList:SetDisabled(Disabled)
            PlayerList.Disabled = Disabled
            if Disabled then
                CloseStatusDropdown()
            end
        end
        ...
        if Info.AutoRefresh ~= false then
            ...
            Library:GiveSignal(Players.PlayerRemoving:Connect(function(Player)
                ...
                if PlayerList.Selected == Player then
                    PlayerList:SetSelected(nil, true)
                end
        ...
        Blank = Groupbox:AddBlank(3, PlayerList.Visible)
        Groupbox:Resize()
